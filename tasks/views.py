...
def signin(request):
    if request.method == 'GET':
        ...
        if user is None:
            return render(request, 'signin.html', {"form": AuthenticationForm, "error": "Usuario o contraseñas incorrectos"})
...
@login_required
def tasks(request):
    #Esta consulta lista las tareas por el usario autenticado y que no este completas, es decir que su datecompleted este vacio
...
def createTask(request):
    if request.method == "GET":
        ...
        try:
            ...
            return render(request,'create-tasks.html',{
            "form":TaskForm,
            "error":"Por favor coloca datos validos"
        })
...
def task_detail(request, task_id):
    if request.method == "GET":
        ...
        try:
            ...
            return render(request,'task-detail.html',{"task":task,"form":form,"error":"Ha ocurrido un error al actualizar la tarea"})
